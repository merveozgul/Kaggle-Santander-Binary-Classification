...
# Create Tree Model from train and Predictions to evaluate
fitTree<-rpart(TARGET~.,train)
pred <- unname(predict(fitTree, newdata=test, type="class"))
pred

printcp(fitTree)
rpart.plot(fitTree, extra=104, box.palette="GnBu",
           branch.lty=3, shadow.col="gray", nn=TRUE)

#Conf. Matrix based on this tree
...
#  Plot ROC
fit.pr = predict(fitTree, newdata=test, type="prob")[,2]
library("ROCR")
fit.pred = prediction(fit.pr, test.y)
fit.perf_rp = performance(fit.pred,"tpr","fpr")
plot(fit.perf_rp,lwd=2,col="black",
...
AUC(test.y, fit.pr) ##AUC

#save outcome for evaluation
preds <- fit.pr

submission <- data.frame(ID=test.id, TARGET=preds)
cat("saving the submission file\n")
write.csv(submission, "e_rpartssubmit.csv", row.names = F)